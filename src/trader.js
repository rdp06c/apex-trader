...
Remember: You're managing real money to MAXIMIZE returns through INFORMED decisions AND teaching your user. Back every decision with research. Be specific. Explain your conviction.`
        ...
        async function updateUI() {
            ...
            // Hard cap: keep at most 3000 entries
            if (portfolio.performanceHistory.length > 3000) {
                portfolio.performanceHistory = portfolio.performanceHistory.slice(-3000);
            }
        ...
        async function savePortfolio(localOnly = false) {
            // Cap unbounded arrays to prevent storage bloat
            const TX_CAP = 500;
            if (portfolio.transactions.length > TX_CAP) {
                portfolio.transactions = portfolio.transactions.slice(-TX_CAP);
            }
            const CLOSED_CAP = 300;
            if ((portfolio.closedTrades || []).length > CLOSED_CAP) {
                portfolio.closedTrades = portfolio.closedTrades.slice(-CLOSED_CAP);
            }
            const PERF_CAP = 3000;
            if ((portfolio.performanceHistory || []).length > PERF_CAP) {
