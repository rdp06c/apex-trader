        ...
        async function runCalibrationSweep(startDateStr, endDateStr, progressCallback) {
            ...
            try {
                ...
                if (revExcursionProfile) {
                    const fmtSeg = (name, s) => {
                        if (!s || s.insufficient) return `  ${name.padEnd(12)}| n=${s?.n ?? 0} (insufficient)`;
                        const st = s.strategy;
                        return `  ${name.padEnd(12)}| n=${String(s.n).padStart(5)} | MAE ${String(s.medianMAE10d).padStart(6)}% | MFE40 ${s.medianMFE40d != null ? ('+' + s.medianMFE40d).padStart(6) : '  N/A '}% | hit10% ${String(s.pctMFE10).padStart(5)}% | hit20% ${String(s.pctMFE20).padStart(5)}% | WR ${st?.winRate ?? 'N/A'}% avg ${st?.avgReturn ?? 'N/A'}%`;
                    };
