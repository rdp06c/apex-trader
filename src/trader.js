        ...
        async function fetchGroupedDailyBars(symbolSet) {
            ...
            // Compute 48 most recent weekdays (skip Sat/Sun) — extra buffer for ~10 US holidays/year
            // We need 40 actual trading days; holidays return 0 results and get skipped
            const tradingDates = [];
            const d = new Date();
            d.setHours(0, 0, 0, 0);
        ...
        function calculateCompositeScore({ momentumScore, rsNormalized, sectorFlow, structureScore, isAccelerating, upDays, totalDays, todayChange, totalReturn5d, rsi, macdCrossover, daysToCover, volumeTrend, fvg, signalAdjustments }) {
            ...
            const compositeScore = momentumContrib + rsContrib + sectorBonus + accelBonus + consistencyBonus
                + structureBonus + extensionPenalty + pullbackBonus + runnerPenalty + declinePenalty
                + rsiBonusPenalty + macdBonus + rsMeanRevPenalty + squeezeBonus + volumeBonus + fvgBonus + learnedAdj;
...
OBJECTIVE: Maximize portfolio returns. Your job is to protect capital by exiting broken setups and preserve winners by holding strong positions. Every dollar tied up in a weak holding is a dollar not working toward better opportunities.
...
⚠️ OPPORTUNITY COST CHURN — OUR #1 PERFORMANCE KILLER:
32% of exits were "opportunity cost" sells, and they are essentially random outcomes (coin flip).
RULE: Do NOT sell a holding to "free up cash" unless the holding has a BROKEN thesis (CHoCH, failed catalyst, structural breakdown). "Something else scores higher" is NOT a valid sell reason. Holding a mediocre position is better than churning into a new one that statistically won't outperform.
...
Code enforces a 15% max position size. Plan your share counts accordingly.
No single stock should exceed 15% of total portfolio value (existing + new shares).
This cap cannot be overridden regardless of conviction level.
...
Remember: You're managing real money to MAXIMIZE returns through INFORMED decisions AND teaching your user. Back every decision with research. Be specific. Explain your conviction.`
        ...
        async function executeMultipleTrades(response, marketData) {
            ...
            // Step 2c: Enforce maximum position size (15% of portfolio value)
            ...
            const maxPositionValue = estimatedPortfolioValue * (MAX_POSITION_PCT / 100);
            buyDecisionsAll = buyDecisionsAll.filter(d => {
                ...
                if (currentValue + addedValue > maxPositionValue && maxPositionValue > 0) {
                    ...
                    if (allowedShares > 0) {
                        ...
                        d.reasoning = (d.reasoning || '') + ` (capped to ${allowedShares} shares — ${MAX_POSITION_PCT}% max position)`;
                        addActivity(`📏 ${d.symbol} capped to ${allowedShares} shares (${MAX_POSITION_PCT}% position limit)`, 'warning');
                        return true;
                    }
                    addActivity(`📏 ${d.symbol} blocked — would exceed ${MAX_POSITION_PCT}% position limit`, 'warning');
                    return false;
                }
                return true;
            });
