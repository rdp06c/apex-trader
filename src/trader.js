        ...
        async function testDataFetch() {
            ...
            try {
                ...
                // Fetch server-computed indicators for top candidates (RSI, MACD, SMA50 from Massive API)
                ...
                try {
                    ...
                    if (spyPrice > 0) {
                        ...
                        const currentValue = await calculatePortfolioValue().then(r => r.total);
                        ...
                        const portfolioReturn = portfolio.totalDeposits > 0
                            ? ((currentValue - portfolio.totalDeposits) / portfolio.totalDeposits) * 100 : 0;
...
Remember: You're managing real money to MAXIMIZE returns through INFORMED decisions AND teaching your user. Back every decision with research. Be specific. Explain your conviction.`
        ...
        function savePortfolio(localOnly = false) {
            // Cap unbounded arrays to prevent localStorage quota exhaustion
            const TX_CAP = 500;
...
- Recent Transactions: ${JSON.stringify(recentTransactions)}`,
        ...
        const scorecardState = { page: 1, filterSector: '' };
        ...
        function updateCandidateScorecard() {
            ...
            html += '<div class="scorecard-table-wrap"><table class="scorecard-table"><thead><tr>' +
                ...
                '<th title="RSI oscillator (0-100). Below 40 is a strong buy signal (+1.5 to +2.5). Above 70 is danger (-3.0). Above 80 is severe (-5.0 and entry multiplier drops to 0.3x). Biggest single unconditional bonus in the system.">RSI</th>' +
                '<th title="MACD crossover state. Bullish ▲ = +2.5, Bearish ▼ Cross = -2.0, Bearish ▼ = -0.5. A 4.5-point swing between bullish and bearish crossover — one of the largest score differentiators.">MACD</th>' +
                ...
                '<th title="Days to Cover (short interest). DTC >5 with Bullish structure = +1.5 squeeze bonus. DTC >3 with structure = +0.75. High DTC without Bullish structure gets nothing — structure is the gatekeeper.">DTC</th>' +
                ...
                '<th title="Market capitalization.">MCap</th>' +
                '</tr></thead><tbody>';
