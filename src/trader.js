...
Remember: You're managing real money to MAXIMIZE returns through INFORMED decisions AND teaching your user. Back every decision with research. Be specific. Explain your conviction.`
        ...
        function updateLearningInsightsDisplay() {
            ...
            if (cal?.weights) {
                ...
                html += `<div style="background:rgba(46,204,64,0.1);border:1px solid rgba(46,204,64,0.3);border-radius:8px;padding:10px 14px;margin-bottom:12px;font-size:0.85em;">
                    <span style="color:#2ecc40;font-weight:600;">&#x1f4ca; Calibrated Weights Active</span>
                    <span style="color:rgba(255,255,255,0.7);margin-left:6px;">— ${calDate} (${cal.dataPoints.toLocaleString()} obs, <span style="color:${improvColor}">${improvStr} validation improvement</span>${regime}${warn})</span>
                </div>`;
            } else {
        ...
        async function runCalibrationSweep(startDateStr, endDateStr, progressCallback) {
            ...
            // Default date range: 6 months ago to 21 trading days ago (need forward data)
            const today = new Date();
        ...
        async function sendMessage() {
            ...
            if (calMatch) {
                try {
                    ...
                    const rangeMsg = startDate ? `${startDate} to ${endDate}` : 'last 6 months';
