        ...
        function evaluateComboHeat(candidate) {
            ...
            for (const def of SIGNAL_COMBO_DEFS) {
                ...
                if (result.vsBaselineReturn > 0.5) {
                    hotCombos.push({ label: result.label, avgReturn10d: result.avgReturn10d, winRate10d: result.winRate10d, n: result.n });
                } else if (result.vsBaselineReturn < -0.5) {
                    coldCombos.push({ label: result.label, avgReturn10d: result.avgReturn10d, winRate10d: result.winRate10d, n: result.n });
                }
            }
        ...
        function updateCandidateScorecard() {
            ...
            pageCandidates.forEach((c, i) => {
                ...
                if (heat && (heat.hotCount > 0 || heat.coldCount > 0)) {
                    ...
                    const tipParts = [];
                    heat.hotCombos.forEach(h => tipParts.push(`🟢 ${h.label}: +${h.avgReturn10d.toFixed(1)}% avg, ${h.winRate10d.toFixed(0)}% WR (${h.n})`));
                    heat.coldCombos.forEach(h => tipParts.push(`🔴 ${h.label}: ${h.avgReturn10d.toFixed(1)}% avg, ${h.winRate10d.toFixed(0)}% WR (${h.n})`));
