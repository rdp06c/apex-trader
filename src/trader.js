...
Remember: You're managing real money to MAXIMIZE returns through INFORMED decisions AND teaching your user. Back every decision with research. Be specific. Explain your conviction.`
        ...
        function buildChatContext(totalValue, priceData) {
            ...
            let spyLine = '';
            if (portfolio.spyBaseline) {
                const spySnap = snapshotCache?.['SPY'];
                const spyPrice = spySnap?.lastTrade?.p || spySnap?.day?.c;
                if (spyPrice) {
                    const spyReturn = ((spyPrice - portfolio.spyBaseline.price) / portfolio.spyBaseline.price * 100).toFixed(2);
            ...
            pageCandidates.forEach((c, i) => {
                ...
                if (sig?.bestMatch) {
                    ...
                    if (pat) {
                        ...
                        const structVal = c.structure || 'unknown';
                        const structScore = c.structureScore ?? 0;
                        const structDisplay = structScore >= 2 && structVal !== 'bullish' && structVal !== 'bullish_continuation'
                            ? `${structVal} (score ${structScore}, CHoCH)` : structVal;
                        const critDetails = {
                            macd: { val: macdDesc, target: 'bullish cross' },
                            rsi: { val: rsiVal, target: '<40' },
                            structure: { val: structDisplay, target: 'bullish or score ≥2' },
                            pullback: { val: ret5dVal, target: '-2% to -8%' }
                        };
                        for (const cr of ENTRY_SIGNAL_PATTERNS[0].criteria) {
                            const hit = pat.criteria[cr.id];
                            const d = critDetails[cr.id];
                            const icon = hit ? '✓' : '✗';
                            tipLines.push(`${icon} ${cr.label}: ${d.val} (need ${d.target})`);
                        }
                    }
