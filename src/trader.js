        ...
        async function testDataFetch() {
            ...
            try {
                ...
                // Inject server indicators: prefer lastCandidateScores from Pi server scan,
                // then fall back to fetching from Polygon API for uncovered symbols.
                // This avoids 1,620 browser API calls (540×3) that took 7+ min due to
                // the browser's 6-connection-per-host limit.
                let drInjected = 0;
                const serverScoreMap = {};
                (portfolio.lastCandidateScores?.candidates || []).forEach(c => {
                    if (c.serverRsi != null || c.serverMacd || c.serverSma50 != null) {
                        serverScoreMap[c.symbol] = c;
                    }
                });

                // Inject from server scan cache first
                dryRunScored.forEach(scored => {
                    const cached = serverScoreMap[scored.symbol];
                    if (cached) {
                ...
                if (drInjected > 0) console.log(`📊 Injected server indicators for ${drInjected} stocks from server scan cache`);

                // Fetch remaining from Polygon API (top candidates + holdings not covered by server scan)
                const coveredSymbols = new Set(Object.keys(serverScoreMap));
