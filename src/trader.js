...
Remember: You're managing real money to MAXIMIZE returns through INFORMED decisions AND teaching your user. Back every decision with research. Be specific. Explain your conviction.`
        ...
        function updateCandidateScorecard() {
            ...
            // Build legend with live calibration strength data
            const calCombos = portfolio.calibratedWeights?.signalCombos?.combos;
            const calEdge = (key) => {
                const c = calCombos?.[key];
                if (!c || c.insufficient || (c.n ?? 0) < 100) return '';
                return ` <span style="color:${c.vsBaselineReturn > 0 ? 'var(--green)' : 'var(--red)'}">(${c.vsBaselineReturn >= 0 ? '+' : ''}${c.vsBaselineReturn.toFixed(1)}%)</span>`;
            };
            // REV uses RSI<40 + Bull Structure as representative combo
            const revEdge = calCombos?.['rsi_low_structure_bull'] ? calEdge('rsi_low_structure_bull') : '';
            const setupRankings = `REV reversal${revEdge} · QMO quiet momentum${calEdge('vol_ratio_low_mom')} · MOM momentum${calEdge('momentum_trend_confirm')} · LDR sector leader${calEdge('sector_leader_mom')} · SQZ squeeze${calEdge('vol_ratio_high_struct')}`;
