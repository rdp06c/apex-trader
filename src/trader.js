...
Remember: You're managing real money to MAXIMIZE returns through INFORMED decisions AND teaching your user. Back every decision with research. Be specific. Explain your conviction.`
        ...
        async function runCalibrationSweep(startDateStr, endDateStr, progressCallback) {
            ...
            for (const key of componentKeys) {
                ...
                // Quintile analysis (against 5d returns — matches typical hold time)
                const sorted = trainingObs.map((o, i) => ({ val: values[i], ret: o.return5d ?? 0 })).sort((a, b) => a.val - b.val);
            ...
            function calibrateWeightSet(observations) {
                ...
                const extensionCorr = componentCorrelations.extensionPenalty?.corr5d || 0;
            ...
            function evaluateWithWeights(weightsToUse, observations) {
                ...
                for (const obs of Object.values(dateGroups)) {
                    ...
                    const rescored = obs.map(o => ({
                        return5d: o.return5d ?? 0,
                        newScore: calculateCompositeScore(o.scoreInputs).total
                    }));
                    rescored.sort((a, b) => b.newScore - a.newScore);
                    rescored.slice(0, 25).forEach(o => { totalReturn += o.return5d; count++; });
                }
                portfolio.calibratedWeights = savedCalibration;
                return count > 0 ? totalReturn / count : 0;
            }

            const defaultAvg5d = evaluateWithWeights(DEFAULT_WEIGHTS, validationObs);
            const calibratedAvg5d = evaluateWithWeights(calibratedWeights, validationObs);

            const improvement = calibratedAvg5d - defaultAvg5d;
            ...
            const sortedCorr = Object.entries(componentCorrelations)
                .sort((a, b) => Math.abs(b[1].corr5d) - Math.abs(a[1].corr5d));
            ...
            portfolio.calibratedWeights = {
                ...
                validation: {
                    calibratedAvg5d: parseFloat(calibratedAvg5d.toFixed(2)),
                    defaultAvg5d: parseFloat(defaultAvg5d.toFixed(2)),
        ...
        async function sendMessage() {
            ...
            if (calMatch) {
                try {
                    ...
                    response += `Validation: top-25 avg 5d return ${result.validation.calibratedAvg5d}% (calibrated) vs ${result.validation.defaultAvg5d}% (default)\n`;
                    ...
                    result.sortedCorr.slice(0, 5).forEach(([key, data]) => {
                        response += `  ${key}: r=${data.corr5d} (top quintile: ${data.topQuintileAvg}%, bottom: ${data.bottomQuintileAvg}%)\n`;
                    });
