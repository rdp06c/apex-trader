...
Remember: You're managing real money to MAXIMIZE returns through INFORMED decisions AND teaching your user. Back every decision with research. Be specific. Explain your conviction.`
        ...
        async function executeMultipleTrades(response, marketData) {
            ...
            const minCashPct = regime === 'bull' ? 10 : regime === 'choppy' ? 20 : 30; // bear = 30%
            ...
            if (portfolio.cash > minCashReserve) {
                ...
                console.log(`💰 Regime ${regime}: cash $${portfolio.cash.toFixed(2)} already below ${minCashPct}% reserve ($${minCashReserve.toFixed(2)}) — no new buys`);
                if (buyDecisionsAll.length > 0) {
                    addActivity(`💰 ${regime.toUpperCase()} regime: holding cash (${minCashPct}% reserve). Skipping ${buyDecisionsAll.length} buy(s).`, 'info');
                    buyDecisionsAll = [];
                }
            }
            ...
            const effectiveBudget = Math.min(portfolio.cash, deployableCash > 0 ? deployableCash : portfolio.cash);
        ...
        async function runCalibrationSweep(startDateStr, endDateStr, progressCallback) {
            ...
            // Default date range: 12 months ago to 21 trading days ago (need forward data)
            const today = new Date();
            const endDefault = new Date(today);
            let forwardBuffer = 0;
            while (forwardBuffer < 21) { endDefault.setDate(endDefault.getDate() - 1); if (endDefault.getDay() !== 0 && endDefault.getDay() !== 6) forwardBuffer++; }
            ...
            const step = Math.max(1, Math.floor(allWeekdays.length / NUM_DATES));
            const calibrationDates = [];
            for (let i = 0; i < allWeekdays.length && calibrationDates.length < NUM_DATES; i += step) {
                calibrationDates.push(allWeekdays[i]);
            }
