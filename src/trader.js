                ...
                for (const [symbol, shares] of Object.entries(portfolio.holdings)) {
                    ...
                    detailHtml += `
                        <div class="holding-item holding-card">
                            ...
                            ${(() => {
                                // Sell signals: ATR stop, Fib targets, Volume divergence
                                // Prefer scanner readings from Pi, fall back to browser calculation
                                const sr = scannerReadings[symbol];
                                const bars = multiDayCache[symbol];
                                const atr = sr?.atr ?? calculateATR(bars);
                                const atrStop = sr?.atrStop ?? (atr && avgPurchasePrice > 0 ? Math.round((avgPurchasePrice - 2 * atr) * 100) / 100 : null);
                                const volDiv = sr?.volumeDivergence ?? detectVolumeDivergence(bars);
                                const fib = sr?.fibTargets ?? calculateFibTargets(bars);

                                let signals = '';

                                // ATR Stop
                                if (atrStop) {
                                    const belowStop = stockPrice.price <= atrStop;
                                    const atrClass = belowStop ? 'negative' : '';
                                    signals += '<span class="holding-card-footer-label">ATR Stop:</span> <span class="holding-card-footer-value ' + atrClass + '">$' + atrStop.toFixed(2) + '</span>';
                                    if (atr) signals += ' <span class="text-muted">(ATR $' + atr.toFixed(2) + ')</span>';
                                    if (belowStop) signals += ' <span class="negative" style="font-weight:600">BREACHED</span>';
                                }

                                // Fib Targets
                                if (fib && fib.type === 'bullish') {
                                    const atFib1 = stockPrice.price >= fib.fib1272;
                                    const atFib2 = stockPrice.price >= fib.fib1618;
                                    signals += '<span class="holding-card-footer-label" style="margin-left:12px">Fib:</span>';
                                    signals += ' <span class="holding-card-footer-value ' + (atFib1 ? 'positive' : '') + '">1.27× $' + fib.fib1272.toFixed(2) + '</span>';
                                    signals += ' · <span class="holding-card-footer-value ' + (atFib2 ? 'positive' : '') + '">1.62× $' + fib.fib1618.toFixed(2) + '</span>';
                                    if (atFib2) signals += ' <span class="positive" style="font-weight:600">TARGET HIT</span>';
                                    else if (atFib1) signals += ' <span class="positive" style="font-weight:600">NEAR TARGET</span>';
                                }

                                // Volume Divergence
                                if (volDiv?.divergence) {
                                    const divClass = volDiv.direction === 'bearish' ? 'negative' : 'positive';
                                    signals += '<span class="holding-card-footer-label" style="margin-left:12px">Vol Div:</span>';
                                    signals += ' <span class="' + divClass + '" style="font-weight:600">' + volDiv.direction.toUpperCase() + '</span>';
                                    signals += ' <span class="text-muted">(price ' + (volDiv.priceTrend > 0 ? '+' : '') + volDiv.priceTrend + '%, vol ' + (volDiv.volumeTrend > 0 ? '+' : '') + volDiv.volumeTrend + '%)</span>';
                                }

                                if (!signals) return '';
                                return '<div class="holding-card-signals">' + signals + '</div>';
                            })()}
                            ${(() => {
