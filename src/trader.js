        ...
        function calculateCompositeScore({ momentumScore, rsNormalized, sectorFlow, structureScore, isAccelerating, upDays, totalDays, todayChange, totalReturn5d, rsi, macdCrossover, daysToCover, volumeTrend, fvg, signalAdjustments, sma20, currentPrice, smaCrossover }) {
            ...
            // [2] RS mean-reversion penalty: RS=100 has 18% win rate in our data — doubled from original
            const rsMeanRevPenalty = rsNormalized >= 9.5 ? -6 : rsNormalized >= 9 ? -4 : rsNormalized >= 8.5 ? -2 : 0;
            ...
            // Enhanced volume analysis: divergences between momentum and volume
            ...
            // SMA20 proximity: near SMA20 = good entry, far above = extended
            let smaProximityBonus = 0;
            if (sma20 != null && currentPrice != null && sma20 > 0) {
                const pctFromSMA20 = ((currentPrice - sma20) / sma20) * 100;
                if (pctFromSMA20 >= 0 && pctFromSMA20 <= 3 && (structureScore ?? 0) >= 1) smaProximityBonus = 2.0;
                else if (pctFromSMA20 < 0 && pctFromSMA20 >= -3 && (structureScore ?? 0) >= 1) smaProximityBonus = 1.0;
                else if (pctFromSMA20 > 15) smaProximityBonus = -1.5;
                else if (pctFromSMA20 > 10) smaProximityBonus = -0.5;
            }

            // SMA 20/50 crossover
            const smaCrossoverBonus = smaCrossover?.crossover === 'bullish' ? 2.0
                : smaCrossover?.crossover === 'bearish' ? -2.0
                : 0;
            ...
            // Entry quality multiplier — penalize extended entries, reward pullbacks (only when score > 0)
            let entryMultiplier = 1.0;
            if (additiveScore > 0) {
                if (rsi != null && rsi > 80 && momentumScore >= 9) entryMultiplier = 0.3;
                else if ((rsi != null && rsi > 70) || momentumScore >= 9 || rsNormalized >= 9) entryMultiplier = 0.6;
                else if (ret5d >= -8 && ret5d <= -1 && (structureScore ?? 0) >= 1) entryMultiplier = 1.3;
            }
...
Remember: You're managing real money to MAXIMIZE returns through INFORMED decisions AND teaching your user. Back every decision with research. Be specific. Explain your conviction.`
        ...
        async function runBacktest(targetDateStr) {
            ...
            // Compute 80 weekdays ending at targetDate (separate from live cache)
            const tradingDates = [];
            ...
            // Fetch historical grouped daily bars (separate cache key to avoid corrupting live data)
            const btBars = {};
            const BATCH_SIZE = 5;
                    ...
                    try {
                        ...
                        if (data.results) {
                            data.results.forEach(bar => {
                                const sym = bar.T;
                                if (!btBars[sym]) btBars[sym] = [];
                                btBars[sym].push({ o: bar.o, h: bar.h, l: bar.l, c: bar.c, v: bar.v, t: bar.t });
                            });
                            datesProcessed++;
                        }
                    } catch (e) { /* skip failed dates */ }
                }));
                if (i + BATCH_SIZE < tradingDates.length) await new Promise(r => setTimeout(r, 100));
            }

            // Sort bars by timestamp
            Object.values(btBars).forEach(bars => bars.sort((a, b) => a.t - b.t));

            // Get the stock universe from screenStocks
            const stockUniverse = screenStocks();
            const allSymbols = stockUniverse.flatMap(s => s.symbols).filter(sym => btBars[sym] && btBars[sym].length >= 20);

            // Build basic market data from last bar
            const marketData = {};
            allSymbols.forEach(sym => {
                const bars = btBars[sym];
            ...
            // Calculate momentum for scoring
            const signalAdj = getSignalAccuracyAdjustments();
            const stockSectors = {};
            const stocksBySector = {};
            stockUniverse.forEach(s => {
                s.symbols.forEach(sym => { stockSectors[sym] = s.sector; });
            });
            Object.entries(marketData).forEach(([sym, data]) => {
                const sector = stockSectors[sym] || 'Unknown';
                if (!stocksBySector[sector]) stocksBySector[sector] = [];
                stocksBySector[sector].push({ symbol: sym, ...data });
            });
            ...
            Object.entries(marketData).forEach(([sym, data]) => {
                ...
                const closes5d = bars.slice(-6);
                const ret5d = closes5d.length >= 2 ? ((closes5d[closes5d.length - 1].c - closes5d[0].c) / closes5d[0].c) * 100 : 0;
                const rsi = calculateRSI(bars);
                const macd = calculateMACD(bars);
                const sma20 = calculateSMA(bars, 20);
                const smaCrossover = calculateSMACrossover(bars);
                ...
                const scoreResult = calculateCompositeScore({
                    momentumScore: Math.min(10, Math.max(0, ret5d / 2 + 5)),
                    rsNormalized: 5,
                    sectorFlow: 'neutral',
                    structureScore: 0,
                    ...
                    upDays: 0,
                    totalDays: 5,
                    todayChange: data.changePercent || 0,
                    totalReturn5d: ret5d,
                    rsi: rsi,
            ...
            // Fetch actual prices 5/10/20 trading days after targetDate
            const futureOffsets = [5, 10, 20];
            const futureDates = [];
            const fd = new Date(targetDate);
            let tradingDaysAhead = 0;
            while (tradingDaysAhead < 21) {
                fd.setDate(fd.getDate() + 1);
                if (fd.getDay() !== 0 && fd.getDay() !== 6) {
                    tradingDaysAhead++;
                    if (futureOffsets.includes(tradingDaysAhead)) {
                        futureDates.push({ offset: tradingDaysAhead, date: `${fd.getFullYear()}-${String(fd.getMonth() + 1).padStart(2, '0')}-${String(fd.getDate()).padStart(2, '0')}` });
                    }
                }
            }

            // Fetch future prices from grouped daily
            const futurePrices = {};
            for (const { offset, date } of futureDates) {
                try {
                    const resp = await fetch(`https://api.polygon.io/v2/aggs/grouped/locale/us/market/stocks/${date}?adjusted=true&apiKey=${POLYGON_API_KEY}`);
                    const data = await resp.json();
                    if (data.results) {
                        data.results.forEach(bar => {
                            if (!futurePrices[bar.T]) futurePrices[bar.T] = {};
                            futurePrices[bar.T][`day${offset}`] = bar.c;
                        });
                    }
                } catch (e) { /* skip */ }
            ...
            // Compute hypothetical returns
            const results = top25.map(({ symbol, score, price }) => {
                const fp = futurePrices[symbol] || {};
                return {
                    symbol,
                    score: score.toFixed(1),
                    entryPrice: price.toFixed(2),
                    return5d: fp.day5 ? (((fp.day5 - price) / price) * 100).toFixed(1) + '%' : 'N/A',
                    return10d: fp.day10 ? (((fp.day10 - price) / price) * 100).toFixed(1) + '%' : 'N/A',
                    return20d: fp.day20 ? (((fp.day20 - price) / price) * 100).toFixed(1) + '%' : 'N/A'
                };
            });

            // Compute summary stats
            const validReturns5d = results.filter(r => r.return5d !== 'N/A').map(r => parseFloat(r.return5d));
            const validReturns10d = results.filter(r => r.return10d !== 'N/A').map(r => parseFloat(r.return10d));
            const avgReturn5d = validReturns5d.length > 0 ? (validReturns5d.reduce((a, b) => a + b, 0) / validReturns5d.length).toFixed(1) : 'N/A';
            const avgReturn10d = validReturns10d.length > 0 ? (validReturns10d.reduce((a, b) => a + b, 0) / validReturns10d.length).toFixed(1) : 'N/A';
            const winRate5d = validReturns5d.length > 0 ? ((validReturns5d.filter(r => r > 0).length / validReturns5d.length) * 100).toFixed(0) : 'N/A';

            // Save to portfolio
            if (!portfolio.backtestResults) portfolio.backtestResults = [];
            portfolio.backtestResults.push({
                targetDate: targetDateStr,
                runDate: new Date().toISOString(),
                summary: { avgReturn5d, avgReturn10d, winRate5d, datesProcessed, symbolsScored: scored.length },
                top25: results
            });
            while (portfolio.backtestResults.length > 20) portfolio.backtestResults.shift();
            savePortfolio(true);

            return { targetDate: targetDateStr, datesProcessed, symbolsScored: scored.length, avgReturn5d, avgReturn10d, winRate5d, top25: results };
        }
