...
Remember: You're managing real money to MAXIMIZE returns through INFORMED decisions AND teaching your user. Back every decision with research. Be specific. Explain your conviction.`
        ...
        function buildChatContext(totalValue, priceData) {
            ...
            const displayed = holdingLines.slice(0, maxHoldings);
            if (displayed.length > 0) {
                let holdingSection = 'Holdings:\n' + displayed.map(h => h.line).join('\n');
                if (holdingLines.length > maxHoldings) holdingSection += `\n...and ${holdingLines.length - maxHoldings} more positions`;
                sections.push(holdingSection);
            }
...
- **AVOID**: Anti-pattern — should not have been entered. Flag for immediate review.`;
        ...
        const scorecardState = { page: 1, filterSector: '', filterHeld: false, filterSignal: '', filterWatchlist: false, filterZone: false, overrideRegimeGate: false, sortField: 'score', sortDir: 'desc' };
