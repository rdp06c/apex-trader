        ...
        const SIGNAL_COMBO_DEFS = [
            ...
            // Setup-type discovery combos — tested via calibration to validate non-reversal patterns
            { id: 'momentum_trend_confirm', label: 'Mom 5-8 + Structure + Volume', tier: 2, filter: s => (s.momentumScore ?? 0) >= 5 && (s.momentumScore ?? 0) <= 8 && (s.structureScore ?? 0) >= 2 && (s.volumeTrend ?? 1) > 1.1 },
            { id: 'breakout_sma_vol', label: 'SMA Bull Cross + Vol>1.3x + Accel', tier: 2, filter: s => s.smaCrossover?.crossover === 'bullish' && (s.volumeRatio ?? 0) > 1.3 && s.isAccelerating },
            { id: 'sector_leader_mom', label: 'RS>70 + Inflow + Mom 3-7', tier: 2, filter: s => (s.rsNormalized ?? 0) > 7 && sectorFlowIsInflow(s.sectorFlow) && (s.momentumScore ?? 0) >= 3 && (s.momentumScore ?? 0) <= 7 },
            { id: 'mean_revert_vol', label: 'Ret5d<-5% + Vol>1.5x + Not Bearish', tier: 1, filter: s => (s.totalReturn5d ?? 0) < -5 && (s.volumeRatio ?? 0) > 1.5 && (s.structureScore ?? 0) >= 0 },
        ];
        ...
        function evaluateComboHeat(candidate) {
            ...
            for (const def of SIGNAL_COMBO_DEFS) {
                ...
                if (!result || result.insufficient) continue;
        ...
        function deriveTradingRules() {
            ...
            const patternDefs = [
                ...
                {
                    id: 'low_composite',
                    label: 'Low Composite Score (<7)',
                    losingFilter: t => t.entryTechnicals && t.entryTechnicals.compositeScore != null && t.entryTechnicals.compositeScore < 7,
                    winningFilter: t => t.entryTechnicals && t.entryTechnicals.compositeScore != null && t.entryTechnicals.compositeScore >= 12,
...
Remember: You're managing real money to MAXIMIZE returns through INFORMED decisions AND teaching your user. Back every decision with research. Be specific. Explain your conviction.`
        ...
        function updateCandidateScorecard() {
            ...
            // Compute entry signals and combo heat for all candidates
            candidates.forEach(c => {
                    ...
                    if (scorecardState.filterSignal === 'any') {
                        ...
                        return sig.patterns.some(p => {
                            ...
                            if (p.calibrationKey) {
                                ...
                                return cal && !cal.insufficient && cal.vsBaselineReturn > 0.5;
                            }
                            return true;
                        });
                    }
            ...
            html += `<select onchange="scorecardSetSignalFilter(this.value)" class="scorecard-sector-filter" style="min-width:100px;margin-right:12px;"><option value="">All Setups</option><option value="any"${scorecardState.filterSignal === 'any' ? ' selected' : ''}>Any Signal</option><option value="reversal"${scorecardState.filterSignal === 'reversal' ? ' selected' : ''}>REV</option><option value="momentum_cont"${scorecardState.filterSignal === 'momentum_cont' ? ' selected' : ''}>MOM</option><option value="breakout"${scorecardState.filterSignal === 'breakout' ? ' selected' : ''}>BRK</option><option value="squeeze"${scorecardState.filterSignal === 'squeeze' ? ' selected' : ''}>SQZ</option><option value="sector_leader"${scorecardState.filterSignal === 'sector_leader' ? ' selected' : ''}>LDR</option><option value="avoid"${scorecardState.filterSignal === 'avoid' ? ' selected' : ''}>AVOID</option></select>`;
            ...
            if (data.timestamp) {
                ...
                html += `<span style="font-size:0.75rem;color:#666;margin-left:8px;" title="${new Date(data.timestamp).toLocaleString()}">Scanned ${ageStr}</span>`;
            }
            if (totalPages > 1) {
            ...
            html += '<div class="scorecard-table-wrap"><table class="scorecard-table"><thead><tr>' +
                ...
                sh('sig', "Setup type signal. REV=reversal, MOM=momentum continuation, BRK=breakout, SQZ=squeeze, LDR=sector leader, AVOID=exhausted runner. Non-reversal patterns gated by calibration.", 'Sig') +
                ...
                sh('mom', "Momentum (0-10). Sweet spot is 3-7: strong trend without penalties. 9+ triggers extension penalty (-3.5) and reduces entry multiplier to 0.6x.", 'Mom') +
            ...
            pageCandidates.forEach((c, i) => {
                ...
                // Entry signal badge — shows setup type (REV/MOM/BRK/SQZ/LDR/AVOID)
                const sig = c._entrySignal;
                let sigBadge = '';
                if (sig) {
                    const comboResults = portfolio.calibratedWeights?.signalCombos?.combos;

                    // Find the best valid (calibration-gated) pattern match
                    let displayPat = null;
                    for (const pat of sig.patterns) {
                        if (!pat.match || pat.antiPattern) continue;
                        // Non-reversal patterns require calibration validation
                        if (pat.calibrationKey) {
                            const calData = comboResults?.[pat.calibrationKey];
                            if (!calData || calData.insufficient || calData.vsBaselineReturn <= 0.5) continue;
                        }
            ...
            html += `<div style="font-size:10px;color:var(--text-muted);margin-top:4px;padding:6px 8px;background:var(--bg-surface);border-radius:4px;border-left:3px solid var(--accent)">Sig: <span class="entry-badge full setup-reversal" style="display:inline">REV</span> reversal, <span class="entry-badge full setup-momentum_cont" style="display:inline">MOM</span> momentum, <span class="entry-badge full setup-breakout" style="display:inline">BRK</span> breakout, <span class="entry-badge full setup-squeeze" style="display:inline">SQZ</span> squeeze, <span class="entry-badge full setup-sector_leader" style="display:inline">LDR</span> sector leader, <span class="entry-badge avoid" style="display:inline">AVOID</span> exhausted. Non-REV patterns require calibration validation. Heat: <span class="heat-dot hot" style="display:inline-block"></span> hot / <span class="heat-dot cold" style="display:inline-block"></span> cold combo, <span style="color:var(--green)">+1.2</span> net edge. Score: <span class="score-driver sig" style="display:inline">S</span> signal / <span class="score-driver mom" style="display:inline">M</span> momentum driven.</div>`;
            container.innerHTML = html;
        }
