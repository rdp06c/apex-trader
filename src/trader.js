...
Remember: You're managing real money to MAXIMIZE returns through INFORMED decisions AND teaching your user. Back every decision with research. Be specific. Explain your conviction.`
        ...
        function updateCandidateScorecard() {
            ...
            html += `<label style="display:inline-flex;align-items:center;gap:4px;font-size:0.8rem;color:#888;cursor:pointer;"><input type="checkbox" onchange="scorecardToggleHeld()"${scorecardState.filterHeld ? ' checked' : ''} style="accent-color:#00d4aa;cursor:pointer;"> Holdings only</label>`;
            html += `<select onchange="scorecardSetSignalFilter(this.value)" class="scorecard-sector-filter" style="min-width:100px;margin-right:4px;"><option value="">All Setups</option><option value="any"${scorecardState.filterSignal === 'any' ? ' selected' : ''}>Any Signal</option><option value="reversal"${scorecardState.filterSignal === 'reversal' ? ' selected' : ''}>REV</option><option value="momentum_cont"${scorecardState.filterSignal === 'momentum_cont' ? ' selected' : ''}>MOM</option><option value="quiet_momentum"${scorecardState.filterSignal === 'quiet_momentum' ? ' selected' : ''}>QMO</option><option value="squeeze"${scorecardState.filterSignal === 'squeeze' ? ' selected' : ''}>SQZ</option><option value="sector_leader"${scorecardState.filterSignal === 'sector_leader' ? ' selected' : ''}>LDR</option><option value="avoid"${scorecardState.filterSignal === 'avoid' ? ' selected' : ''}>AVOID</option></select>`;
            html += `<label style="display:inline-flex;align-items:center;gap:4px;font-size:0.8rem;color:#888;cursor:pointer;"><input type="checkbox" onchange="scorecardToggleWatchlist()"${scorecardState.filterWatchlist ? ' checked' : ''} style="accent-color:#f59e0b;cursor:pointer;"> ★ Watchlist${watchlistSet.size > 0 ? ' (' + watchlistSet.size + ')' : ''}</label>`;
            ...
            html += `<div style="font-size:10px;color:var(--text-muted);margin-top:4px;padding:6px 8px;background:var(--bg-surface);border-radius:4px;border-left:3px solid var(--accent);display:flex;flex-wrap:wrap;gap:6px 16px;align-items:center"><span>Sig quality: <span class="entry-badge full" style="display:inline">GREEN</span> all criteria met <span class="entry-badge strong" style="display:inline">YELLOW</span> one miss <span class="entry-badge partial" style="display:inline">GRAY</span> minimum met <span class="entry-badge avoid" style="display:inline">RED</span> anti-pattern</span><span>Types: REV=reversal · MOM=momentum · QMO=quiet momentum · SQZ=squeeze · LDR=sector leader · Non-REV gated by calibration</span><span>Heat: <span class="heat-dot hot" style="display:inline-block"></span> hot / <span class="heat-dot cold" style="display:inline-block"></span> cold combo, <span style="color:var(--green)">+1.2</span> net edge · Score: <span class="score-driver sig" style="display:inline">S</span> signal / <span class="score-driver mom" style="display:inline">M</span> momentum driven</span></div>`;
            container.innerHTML = html;
        }
